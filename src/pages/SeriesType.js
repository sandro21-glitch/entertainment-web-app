...
const SeriesType = () => {
  ...
  const {series,value,addBookmark} = useMoviesContext()
  ...
  return (
    <section className='mx-auto flex flex-col py-6 px-4 md:m-6 md:px-0 md:pt-0 ml-0 lg:ml-32'>
      ...
      {series.filter((serie) => {
        return value?.toLowerCase() === '' ? serie : serie.title.toLowerCase().includes(value)
      }).map((serie) => {
        const {id,title,category,year,rating,isBookmarked,thumbnail:{regular:{large}}} = serie
        return (
