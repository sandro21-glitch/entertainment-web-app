...
const Bookmarks = () => {
    ...
    const {bookmarks,value,removeBookmark} = useMoviesContext()
  return (
    <section className='mx-auto flex flex-col py-6 px-4 md:m-6 md:px-0 md:pt-0 ml-0 lg:ml-32'>
        <Search location={location} />
        <h2 className='text-xl md:text-2xl lg:text-4xl text-white capitalize mb-5'>
            {`${bookmarks ? `you have ${bookmarks.length} bookmark` : 'You currenly have no bookmark.'}`}
        </h2>
      ...
      {bookmarks.filter((movie) => {
        ...
        const {id,title,isBookmarked,category,year,rating,thumbnail:{regular:{large}}} = movie
        return (
          ...
          <div className='relative mb-1 group/rec max-w-[320px] rounded-xl overflow-hidden transition-all ease-in duration-200'>
              ...
              {/* bookmark */}
              <div onClick={() => removeBookmark(id)} className={`${isBookmarked ? 'fill-white' : 'fill-none'} z-50 group/bookmark absolute top-3 right-3 text-white w-[2rem] h-[2rem] bg-bookmarkBg cursor-pointer rounded-full flex items-center justify-center
