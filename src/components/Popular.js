...
const Popular = () => {
  const {error,loading,popularMovies,addBookmark} = useMoviesContext()
  ...
  return (
    <section>
        <h2 className='capitalize text-xl md:text-3xl text-white tracking-wide mb-5'>
            Recommended for you
          </h2>
          <article className='grid place-items-start grid-cols-2  md:grid-cols-3 lg:grid-cols-4 gap-5 md:gap-x-[2.5rem] md:gap-y-[2.5rem]'>
            {popularMovies.map((movie) => {
              const {id,title,isBookmarked,thumbnail:{regular:{large}},year,category,rating} = movie
              return (
